 ...
 * 1，cbase_DrawablePadding处理原生DrawablePadding对drawableRight和drawableLeft处理的bug
 * 2，处理空心button/textview实现TextView各种shape xml代码实现 cbase_AngleCorner代表四周角度 cbase_SolidColor代表solidColor cbase_StrokeColor表示边框颜色
 * 3，处理实心button/textview实现TextView各种shape xml代码实现 cbase_AngleCorner代表四周角度 cbase_SolidColor代表solidColor cbase_StrokeColor表示边框颜色
 * 4，button/textview中倾斜字体 cbase_RotateDegree表示倾斜度数
 * 5，实现特殊文本效果 常见收益率比如 66% 要求“%” 的字体大小和颜色与 “66”不一样展示 cbase_SpecialTextReg表示要特殊展示的正则 例如 % cbase_SpecialTextSize表示要特殊展示的大小 比如10sp
 * 6，特殊文本的点击事件
 * 7，多行展示的监听
 */
class CTextView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        xsolidColor = a.getColor(R.styleable.CTextView_CSolidColor, xsolidColor)
        ...
        if (pressedColor == Color.TRANSPARENT) {
            if (xsolidColor != Color.TRANSPARENT) {
                pressedColor = xsolidColor
            }
        }
